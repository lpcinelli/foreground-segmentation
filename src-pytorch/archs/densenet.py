...
def adaptPretrainedModel(model):
    # for param in model.parameters():
    #         param.requires_grad = False
    num_features = model.classifier.bias.shape[0]
    num_classes  = model.classifier.weights.shape[0]/num_features
...
def densenet121(pretrained=False, **kwargs):
    ...
    num_classes = kwargs.pop('num_classes', 17)
    if not input_shape:
...
class DenseNet(nn.Module):
    ...
    def __init__(self, input_shape,
        ...
        self.features = nn.Sequential(OrderedDict([
            ('conv0', nn.Conv2d(C, num_init_features, kernel_size=7, stride=2,
