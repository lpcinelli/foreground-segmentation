...
--TODO: add code to divide image in many patchs
...
function CDNetDataset:__init(imageInfo, opt, split)
   ...
   self.hdf5File = hdf5.open(paths.concat(opt.data, opt.dataset .. '.h5'), 'r')
end
...
function CDNetDataset:get(i)
   ...
   local img = self:_loadImage(paths.concat(self.dir, self.imageInfo.imagePath[i]:data()), 3)
   ...
   return {
				input = torch.cat({videoROI:expandAs(img),
...
function CDNetDataset:_loadImage(path, channels)
	-- baseline/highway/input/in000484
   ...
   -- end
   print(path)
   return self.hdf5File:read(path):all()[{{1,channels},{},{}}]
   -- return input
end
