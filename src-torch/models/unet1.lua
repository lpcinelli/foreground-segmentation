...
-- https://gist.github.com/toshi-k/ca75e614f1ac12fa44f62014ac1d6465
--
local nn = require 'nn'
require 'cunn'
...
local function createModel(opt)
   ...
   function branch(insert, inPlanes, outPlanes)
   ...
   function conv(n_input, n_middle, n_output, filtsize, out_bn)
   	...
   	s:add(cudnn.SpatialConvolution(n_input, n_middle, filtsize, filtsize, 1, 1, 1, 1))
   	s:add(nn.SpatialBatchNormalization(n_middle))
   	s:add(nn.LeakyReLU(0.1, true))
   
   	s:add(cudnn.SpatialConvolution(n_middle, n_output, filtsize, filtsize, 1, 1, 1, 1))
   	if out_bn == true then
   		s:add(nn.SpatialBatchNormalization(n_output))
   	end
   ...
   local model = nn.Sequential()
   model:add(conv(2, 64, 64, filtsize, true))
   ...
   -- func returning error at torch.cat
   ...
   if opt.dataset ~= 'cdnet' and opt.dataset ~= 'sanity' then
      model:get(1):get(1).gradInput = nil
   end
